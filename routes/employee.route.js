    ...
    try {
        ...
        if (!employee) {
            ...
            if (isCorrectPassword) {
                ...
                if (!employee.isActive) return res.status(200).send({
                    success:
                ...
                return res.status(200).send({ success: true, employee, token ,refreshToken })
...
const generateRefreshToken=(employee)=> {
    return jwt.sign({ idemployee: employee._id, role: employee.role },
...
router.post('/refreshToken', async (req, res,) => {
    ...
    if (!refreshtoken) {
        ...
        jwt.verify(refreshtoken, process.env.REFRESH_TOKEN_SECRET, (err, employee) => {
            if (err) {
                console.log(err)
...
router.get('/', async (req, res,) => {
    try {
        const employees = await Employee.find({}, null, { sort: { '_id': -1 } })/* .select("-password"); */
...
router.post('/register', async (req, res) => {
    try {
        let { email, password, firstname, lastname,avatar,role,phone } = req.body
        ...
        const newEmployee = new Employee({ email, password, firstname, lastname,avatar,role,phone })
    ...
    here</a>`
        ...
        return res.status(201).send({ success: true, message: "Account created successfully", employee: createdEmployee })
    } catch (err) {
        console.log(err)
